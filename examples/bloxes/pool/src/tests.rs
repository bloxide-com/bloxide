...
mod pool_tests {
    ...
    use bloxide_spawn::{
        peer::PeerCtrl,
        test_impl::{drain_spawned, spawned_count},
        SpawnCap,
    };
    use pool_messages::{PoolMsg, SpawnWorker, WorkDone, WorkerMsg};

    use crate::{PoolCtx, PoolSpec, PoolState};
    ...
    impl PoolHarness {
        ...
        fn dispatch_spawn_worker(&mut self, task_id: u32) {
            self.machine
                .dispatch(Envelope(0, PoolMsg::SpawnWorker(SpawnWorker { task_id })).into());
        }
        ...
        fn current_state(&self) -> Option<PoolState> {
    ...
    fn pool_starts_in_idle() {
        ...
        h.start();
        assert_eq!(h.current_state(), Some(PoolState::Idle));
    }
    ...
    fn spawn_worker_transitions_idle_to_active() {
        ...
        assert_eq!(h.current_state(), Some(PoolState::Active));
        assert_eq!(h.pending(), 1);
        assert_eq!(spawned_count(), 1, "one worker future should be queued");
        drain_spawned();
    }
    ...
    fn multiple_spawn_workers_stay_active() {
        ...
        assert_eq!(spawned_count(), 3, "three worker futures should be queued");
        drain_spawned();
    }
    ...
    fn all_work_done_transitions_to_all_done() {
        ...
        assert_eq!(h.current_state(), Some(PoolState::AllDone));
