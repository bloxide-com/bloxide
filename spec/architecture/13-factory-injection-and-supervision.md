        ...
        + From<Envelope<WorkerMsg>>,
{
    match req {
        SpawnRequest::Worker { reply_to, pool_ref, .. } => {
            ...
            // Spawn the supervised task (lifecycle + abort + notify)
            let task_handle = TokioRuntime::spawn(async move {
                run(machine, (ctrl_rx, domain_rx),
                    RunConfig::supervised_with_abort(lifecycle_rx, abort_rx, notify.sender()),
                    worker_id).await;
            });

            // JoinHandle (not Clone) → KillHandle (Clone) for the ripcord
            ...
            // Return lifecycle/capability handles for the supervisor
            SpawnOutput {
                child_id: worker_id,
                lifecycle_ref,
                abort_ref,
...
- Calls `SpawnCap::spawn` (uses runtime-specific spawning)
...
The generated `main.rs` injects the factory, monomorphizing `spawn_worker` with the system-generated concrete worker spec:
...
1. An Embassy impl crate would define its own `spawn_worker` that:
...
│             │         │             │         │  generated main) │
│ [[context.  │         │ fn          │         │                  │
│  uses]]     │         │ spawn_worker│         │ [actors.inject]  │
│ spawn_fn:   │◄────────│ <S>(req,    │◄────────│ spawn_fn =       │
│ SpawnFn     │         │   notify)   │         │  { source =      │
│ role="ctor" │         │             │         │   "factory" }    │
└─────────────┘         └─────────────┘         └──────────────────┘
       │                                                 │
       ▼                                                 ▼
┌─────────────┐                                 ┌──────────────────┐
│  PoolCtx    │                                 │ PoolCtx::new(.., │
│ plain field │◄────────────────────────────────│  spawn_worker::< │
│ (no traits) │   monomorphized with the        │  ConcreteSpec>)  │
