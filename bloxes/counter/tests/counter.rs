...
// Integration tests for the Counter blox.
//
// Run with: `cargo blox generate`, then
// `cargo test --manifest-path target/bloxide-generated/Cargo.toml -p counter-blox`
...
fn reset_returns_to_ready_without_resetting_count() {
    ...
    assert!(matches!(
        outcome,
        bloxide_core::engine::DispatchOutcome::Started(MachineState::State(CounterState::Ready))
    ));
    assert!(matches!(
        machine.current_state(),
        MachineState::State(CounterState::Ready)
