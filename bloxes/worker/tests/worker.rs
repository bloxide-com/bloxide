...
// Integration tests for the Worker blox.
//
// Run with: `cargo blox generate`, then
// `cargo test --manifest-path target/bloxide-generated/Cargo.toml -p worker-blox`
...
use worker_blox::{WorkerCtx, WorkerEvent, WorkerSpec, WorkerState};
...
fn ctrl_mailbox_is_polled_before_domain_mailbox() {
    ...
    >(worker_id, 16);
    let (msg_ref, msg_rx) = <TestRuntime as DynamicChannelCap>::channel::<WorkerMsg>(worker_id, 16);
