...
Defines traits and free action functions taking concrete params. Zero concrete implementations in blox crates.
...
**State traits** define mutable state (no `#[delegatable]`, no `B` generic):
...
**Free action functions** take concrete params, not trait-bounded `&mut C`:
...
**Important:** There is no `B` generic, no `#[delegatable]`, no `#[delegates]`. Context traits are implemented directly on the context struct by the codegen.
...
Context fields come from `[[context.uses]]` (accessor traits) and `[[context.fields]]` (state fields) entries in `blox.toml`. The codegen auto-emits `self_id` (first field). There is no `B` generic, no `behavior: B` field, no `#[delegates(...)]`. The generated struct looks like:
