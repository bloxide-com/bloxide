...
Context fields come from `[[context.uses]]` (accessor traits) and `[[context.fields]]` (state fields) in `blox.toml`. The codegen auto-emits `self_id` (first field). There is no `B` generic, no `behavior: B` field, no `#[delegates(...)]`.
...
| `peer_ref: ActorRef<M, R>` | `#[provides(HasPeerRef<R, M>)]` |
| `timer_ref: ActorRef<TimerCommand, R>` | `#[provides(HasTimerRef<R>)]` |
| `pool_ref: ActorRef<PoolMsg, R>` | `#[provides(HasPoolRef<R>)]` |
| `worker_factory: fn(...) -> ...` | Constructor parameter (no trait) |
| `round: u32` (from `[[context.fields]]`) | Direct field, zero-initialized |
