...
///
/// The `task_handle` IS here — the spawn function gets it from `R::spawn()`
/// and passes it to the managing blox so it can call `R::Kill::kill(handle)`
...
where
    <R::Kill as KillCapability<R>>::Handle: Clone,
{
    fn clone(&self) -> Self {
        Self {
