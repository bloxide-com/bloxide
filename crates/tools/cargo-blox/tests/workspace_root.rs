...
// `apps/…`, and `Cargo.toml` relative to the current directory.
...
fn assert_registered(cwd: &PathBuf, member: &str, root_toml: &str) {
    ...
    for polluted in ["crates", "apps", "spec", "Cargo.toml"] {
        assert!(
...
fn new_blox_from_subdirectory_scaffolds_at_root() {
    ...
    assert!(dir.path().join("crates/bloxes/demo/blox.toml").exists());
    assert!(dir.path().join("crates/bloxes/demo/Cargo.toml").exists());
    assert!(dir.path().join("crates/bloxes/demo/src/lib.rs").exists());
    assert!(dir.path().join("spec/bloxes/demo.md").exists());
    let root_toml =
        fs::read_to_string(dir.path().join("Cargo.toml")).expect("read root Cargo.toml");
    assert!(
        root_toml.contains("demo-blox = { path = \"crates/bloxes/demo\" }"),
        "workspace dependency should be registered:\n{root_toml}"
    );
    assert_registered(&cwd, "crates/bloxes/demo", &root_toml);
}

#[test]
fn new_impl_from_subdirectory_scaffolds_at_root() {
    let dir = write_scaffold_fixture();
    let blox_dir = dir.path().join("crates/bloxes/demo");
    fs::create_dir_all(&blox_dir).expect("create blox dir");
...
fn new_binary_from_subdirectory_scaffolds_at_root() {
    ...
    assert!(dir.path().join("apps/demo/system.toml").exists());
    let root_toml =
        fs::read_to_string(dir.path().join("Cargo.toml")).expect("read root Cargo.toml");
    assert_registered(&cwd, "apps/demo", &root_toml);
}
...
fn new_all_from_subdirectory_scaffolds_all_layers_at_root() {
    ...
    for path in [
        ...
        "crates/bloxes/demo/blox.toml",
        "crates/impl/demo/src/lib.rs",
        "apps/demo/system.toml",
        "spec/bloxes/demo.md",
    ] {
...
fn new_binary_with_invalid_runtime_fails_without_writing() {
    ...
    assert!(
        !dir.path().join("apps").exists(),
        "no app directory should be scaffolded on validation failure"
    );
    let root_toml_after =
        fs::read_to_string(dir.path().join("Cargo.toml")).expect("read root Cargo.toml");
    assert_eq!(
...
fn new_binary_with_valid_runtimes_scaffolds() {
    for runtime in ["tokio", "embassy"] {
        ...
        let system_toml = fs::read_to_string(dir.path().join("apps/demo/system.toml"))
