...
//! auto-emits the universal `self_id` and `behavior` fields. Every `use`
//! statement is a direct 1:1 mapping from the TOML — the codegen never
//! guesses imports.
//!
...
fn generate_variant(
    ...
    // 0: Auto-emit implicit fields FIRST so they appear first in the struct
    //    (and thus first in the generated `new()` constructor).
    //    self_id: ActorId — always emitted.
    ...
    for u in &config.uses {
        ...
        if let Some(ref field_name) = u.field {
            if let Some(ref field_ty_str) = u.field_type {
                ...
                // Explicit provides/provides_mut take precedence; fall back to
                // the trait name for the provides annotation.
                let provides = u.provides.as_deref().or(u.trait_.as_deref());
                let provides_mut = u.provides_mut.as_deref();
                let attrs = field_attrs(u.role.as_deref(), None, provides, provides_mut);
                field_defs.push(quote! {
                    #(#attrs)*
        ...
        for sub in &u.fields {
            ...
            let attrs = field_attrs(
                sub.role.as_deref(),
                None,
                sub.provides.as_deref(),
                sub.provides_mut.as_deref(),
            );
            field_defs.push(quote! {
                #(#attrs)*
                pub #field_ident: #field_ty,
    ...
    {
        let delegate_traits: Vec<syn::Path> = {
            ...
            for u in &config.uses {
                ...
                if u.delegatable {
                    ...
                    for ts in trait_strs {
                        if let Ok(path) = syn::parse_str::<syn::Path>(ts) {
                            // Keep the full trait path WITH generics for #[delegates(...)].
                            // The BloxCtx macro needs the full path (e.g. HasPeers<WorkerMsg, R>).
                            let no_gen = path_without_generics(&path);
                            if !traits
                                .iter()
                                .any(|t: &syn::Path| path_to_string(t) == path_to_string(&no_gen))
                            {
                                traits.push(path);
                            }
                        }
        ...
        if !delegate_traits.is_empty() {
            let delegate_attr = quote! { #[delegates(#(#delegate_traits),*)] };
            field_defs.push(quote! {
                #delegate_attr
                pub behavior: B,
            });
