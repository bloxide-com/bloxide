...
fn generate_transition_body(
    ...
    if trans.guards.is_empty() {
        ...
        let guard_arms: Vec<proc_macro2::TokenStream> = trans
            ...
            .iter()
            .map(|g| {
                let cond_ts: proc_macro2::TokenStream =
                    g.condition.parse().unwrap_or_else(|_| quote! { false });
...
fn generate_state_fns(
    ...
    if transitions.is_empty() {
        ...
        let arms: Vec<proc_macro2::TokenStream> = transitions
            .iter()
            .map(|t| {
                let event_ts: proc_macro2::TokenStream =
                    t.event.parse().unwrap_or_else(|_| quote! { _ });
