...
pub fn generate(
    ...
    let has_declarative = !config.transitions.is_empty() && !has_feature;
    ...
    let handler_code = if let Some(ref fns) = config.handler_fns {
        ...
        // Declarative mode: transitions are declared in TOML.
        // Generate StateFns constants with raw StateRule { ... } struct literals.
        // This is used for non-feature-gated bloxes. For feature-gated bloxes
        // (supervisor), spec_skeleton.rs generates the StateFns as associated
        // constants inside paired impl blocks.
    ...
    } else if !config.transitions.is_empty() && has_feature {
        // Feature-gated declarative: emit only the handler table macro.
        // StateFns constants are generated by spec_skeleton.rs as associated
        // constants inside paired impl blocks (using transitions! macro).
        let macro_name_str = format!("{}_handler_table", to_snake_case(&enum_name_str));
        let macro_name = format_ident!("{}", macro_name_str);
